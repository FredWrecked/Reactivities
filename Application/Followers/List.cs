...
public class List
{
    ...
    public class Handler: IRequestHandler<Query, Result<List<Profile>>>
    {
        ...
        public async Task<Result<List<Profile>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            {
                case "followers":
                    profiles = await _context.UserFollowings.Where(it => it.Target.UserName == request.Username)
                        .Select(u => u.Observer)
                        .ProjectTo<Profile>(_mapper.ConfigurationProvider, new {currentUsername = _userAccessor.GetUsername()})
                        .ToListAsync();
                    break;
                case "following":
                    profiles = await _context.UserFollowings.Where(it => it.Observer.UserName == request.Username)
                        .Select(u => u.Target)
                        .ProjectTo<Profile>(_mapper.ConfigurationProvider, new {currentUsername = _userAccessor.GetUsername()})
                        .ToListAsync();
                    break;
